# Main file for "Extended Neural Network Generalizability 
...
def dataset_load_split2(dataset_path, batch_size = opt.batch_size, num_workers = 6):
    eval_dataset_size = opt.eval_dataset_size*batch_size
    if opt.eval_dataset_sampler >= 10  and opt.eval_dataset_sampler < 100:
        ...
        train_dataset_sampler = list(range(0, train_size, train_size//opt.dataset_size))
...
class com_loss(torch.nn.Module):
    ...
    def forward(self, x, y, epoch=0):
        return com_loss.MSE(x,y) - com_loss.PCC(x,y) # com_loss combine MSE and PCC
...
def generator_train(train=True, epoch=0):
    ...
    if train: gen_imgs_lpips = torch.zeros(Gen_imgs.shape[0])
    else: gen_imgs_lpips = com_loss.LPIPS(Gen_imgs, image).to('cpu')
...
with tqdm(range(max_epochs)) as tepoch:
    for epoch in tepoch:
        ...
        for i, (speckle,image) in (enumerate(dataloader['train'])): # Train
            ...
            if epoch + 2 >= max_epochs and opt.update_on_final_epoch == 0:
                with torch.no_grad():
                    Gen_imgs, g_loss, gen_imgs_mse, gen_imgs_pcc, gen_imgs_ssim, gen_imgs_psnr, gen_imgs_lpips = generator_train(train=False,epoch=epoch) 
                    d_loss, real_pred, fake_pred = discriminator_train(train=False,  iteration=opt.dis_iteration)
            else:
